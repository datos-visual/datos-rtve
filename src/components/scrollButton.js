import flechaIcon from "../assets/flecha-movil.svg";

class ScrollButton extends HTMLElement {
  static get observedAttributes() { return ["label", "icon", "animated", "distance", "duration", "thickness"]; }
  ...
  disconnectedCallback() { this.cleanupResizeHandler(); }
  ...
  getIconMarkup(wheelStroke) {
    return this.icon === "flecha"
      ? `<img class="scroll-icon" src="${flechaIcon}" alt="" aria-hidden="true" />`
      : `
  ...
  addEventListeners() {
    ...
    this.elements.button.addEventListener("click", () => {
      this.dispatchEvent(new CustomEvent("scroll-button-click", { bubbles: true, composed: true }));
    });
